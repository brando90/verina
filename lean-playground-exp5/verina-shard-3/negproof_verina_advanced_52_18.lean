...
-- The implementation counts elements < k, so it returns 1 (only element 1 is < 2)
have h_result : minOperations [2, 1] 2 h_precond = 1 := by
  simp [minOperations]
  rfl
rw [h_result]
-- Now we need to show the postcondition is false for result = 1
-- processed = [2, 1].reverse.take 1 = [1, 2].take 1 = [1]
-- target_nums = [1, 2]
-- collected_all requires both 1 and 2 to be in [1], but [1] only contains 1
left
simp [List.reverse, List.take, List.range, List.map, List.all, List.elem]
-- The goal is now ¬(True ∧ (2 = 1))
-- We need to show this is false
intro ⟨_, h⟩
exact absurd h (Nat.succ_ne_self 1)
