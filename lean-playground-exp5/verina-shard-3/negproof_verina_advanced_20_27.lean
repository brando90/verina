...
intro h
-- 16 % 8 = 0, so the left side is true
have left_true : 16 % 8 = 0 ∨ ∃ i, Int.natAbs 16 / 10 ^ i % 10 = 8 := by
  left
  norm_num
-- By h.mp, isItEight 16 should be true
have should_be_true := h.mp left_true
-- But isItEight 16 is false because 16 has no digit 8
