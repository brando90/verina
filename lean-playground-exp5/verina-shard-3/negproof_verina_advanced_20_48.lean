...
-- But isItEight 16 actually returns false (since 16 has digits 1 and 6, no 8)
have h4 : isItEight 16 trivial = false := by
  unfold isItEight
  simp only [Int.natAbs_of_nonneg (by norm_num : (16 : Int) ≥ 0)]
  -- We need to evaluate hasDigitEight 16
  -- hasDigitEight is a recursive function, let's trace through it
  -- First call: hasDigitEight 16
  -- 16 ≠ 0, so check if 16 % 10 = 8
  -- 16 % 10 = 6 ≠ 8, so recurse on 16 / 10 = 1
  -- Second call: hasDigitEight 1
  -- 1 ≠ 0, so check if 1 % 10 = 8
  -- 1 % 10 = 1 ≠ 8, so recurse on 1 / 10 = 0
  -- Third call: hasDigitEight 0
  -- 0 = 0, so return false
  native_decide
-- Contradiction
rw [h4] at h3
