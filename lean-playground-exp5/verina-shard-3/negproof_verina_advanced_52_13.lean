...
-- The implementation returns 1 (counts elements < 2, which is just 1)
have h_result : minOperations [2, 1] 2 h_precond = 1 := by
  simp [minOperations]
  rfl
rw [h_result]
-- Now we show the postcondition is false
-- processed = [2, 1].reverse.take 1 = [1, 2].take 1 = [1]
-- target_nums = [1, 2]
-- collected_all requires both 1 and 2 to be in [1]
-- But [1] only contains 1, not 2
left
simp [List.reverse, List.take, List.range, List.map, List.all, List.elem]
-- The goal should now be ¬(1 = 1 ∧ (2 = 1))
-- which simplifies to ¬(True ∧ False) = ¬False = True
simp
