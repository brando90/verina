...
-- minOperations counts elements < 2, which is just 1, so returns 1
have h_result : minOperations [2, 1] 2 _ = 1 := rfl
rw [h_result]
-- processed = [2, 1].reverse.take 1 = [1, 2].take 1 = [1]
-- target_nums = [1, 2]
-- collected_all requires both 1 and 2 to be in [1], but 2 is not in [1]
simp [List.reverse, List.take, List.range, List.map, List.all, List.elem, not_and_or]
