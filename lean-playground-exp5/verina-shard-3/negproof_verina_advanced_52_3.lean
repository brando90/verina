...
-- The implementation counts 1 element (only 1 < 2)
-- But from the reversed list [2, 1], we need 2 elements to get both 1 and 2
simp [List.reverse, List.take, List.range, List.map, List.all, List.elem]
-- The goal should now show that the postcondition is false
decide
