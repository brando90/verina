...
theorem minOperations_spec_violated : ∃ nums k,
  ...
  use [1, 2], 2
have h_pre : minOperations_precond [1, 2] 2 := by
...
simp only [decide_eq_false_iff_not, ite_false]
-- The implementation counts elements < 2, which is just 1
have count_eq : minOperations.count [1, 2] 0 = 1 := by
  simp only [minOperations.count]
  simp only [decide_eq_true_eq, ite_true]
  simp only [minOperations.count]
...
-- reverse [1, 2] = [2, 1], take 1 [2, 1] = [2]
have rev_eq : List.reverse [1, 2] = [2, 1] := by decide
rw [rev_eq]
have take_eq : List.take 1 [2, 1] = [2] := by decide
rw [take_eq]
-- We need to show ¬(collected_all ∧ is_minimal)
-- collected_all requires both 1 and 2 to be in [2]
-- But 1 is not in [2], so collected_all is false
simp only [List.all, List.elem, and_comm]
