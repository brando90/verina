...
theorem minOperations_spec_violated : ∃ nums k,
  ...
  use [3, 1, 2], 2
have h_pre : minOperations_precond [3, 1, 2] 2 := by
  simp only [minOperations_precond]
  simp only [List.range, List.map, List.all, List.elem]
  constructor
  · simp only [List.elem, decide_True, Bool.true_or]
  · simp only [List.elem]
    simp only [decide_False, Bool.false_or, decide_True, Bool.true_or]
use h_pre
simp only [minOperations_postcond, minOperations]
simp only [decide_False, ite_false]
-- The implementation counts elements < 2, which is just 1
have count_eq : minOperations.count [3, 1, 2] 0 = 1 := by
  ...
  simp only [minOperations.count]
rw [count_eq]
-- Show the postcondition is false with result = 1
simp only [List.range, List.map]
have rev_eq : List.reverse [3, 1, 2] = [2, 1, 3] := by rfl
rw [rev_eq]
have take_eq : List.take 1 [2, 1, 3] = [2] := by rfl
rw [take_eq]
-- The collected_all part requires both 1 and 2 to be in the taken elements
-- But we only took [2], which doesn't contain 1
simp only [List.all, List.elem, Bool.not_and]
left
simp only [decide_False, Bool.false_and]
