...
constructor
· -- Prove precondition
  simp [List.range, List.map, List.all, List.elem]
  constructor
  · simp
  · simp
· -- Prove postcondition is violated
  simp [minOperations, minOperations_postcond]
  simp [List.range, List.map, List.reverse, List.take, List.all, List.elem]
  -- The implementation returns 1 (counting element 1 < 2)
  -- But taking 1 element from reversed [1,2] gives [2], which doesn't contain 1
  norm_num
