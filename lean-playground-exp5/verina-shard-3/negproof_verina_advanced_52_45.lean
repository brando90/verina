...
simp only [List.reverse, List.reverseAux, List.take, List.range, List.map, List.all, List.elem]
simp only [decide_True, decide_False, Bool.false_and, Bool.and_false, not_false_iff]
