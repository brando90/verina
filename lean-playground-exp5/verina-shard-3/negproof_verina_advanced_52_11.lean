...
use h_precond
simp only [minOperations_postcond, not_and_or]
-- Compute what minOperations returns
have h_result : minOperations [1, 2] 2 h_precond = 1 := by
  simp [minOperations]
  rfl
rw [h_result]
-- Show either collected_all is false or is_minimal is false
-- We'll show collected_all is false
left
-- processed = [1, 2].reverse.take 1 = [2, 1].take 1 = [2]
-- target_nums = [1, 2]
-- We need to show that not all of [1, 2] are in [2]
