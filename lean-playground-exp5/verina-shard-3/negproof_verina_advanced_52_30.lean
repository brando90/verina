...
-- The implementation returns 1 (since only 1 < 2)
have count_eq : minOperations.count [2, 1] 0 = 1 := by
  simp only [minOperations.count]
  simp only [decide_eq_true_eq, ite_true, decide_eq_false_iff_not, not_lt]
  norm_num
  simp only [minOperations.count]
simp only [count_eq]
-- Now evaluate the postcondition with result = 1
simp only [List.range, List.map]
simp only [List.reverse, List.take]
simp only [List.all, List.elem]
simp only [decide_eq_true_eq, decide_eq_false_iff_not, not_and]
-- We need to show that not (1 ∈ [1] ∧ 2 ∈ [1] ∧ is_minimal)
-- Since 2 ∉ [1], collected_all is false
intro h
cases h
