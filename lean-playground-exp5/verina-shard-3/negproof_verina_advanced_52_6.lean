...
theorem minOperations_spec_violated : ∃ nums k,
  ...
  use [1, 2], 2
have h_precond : minOperations_precond [1, 2] 2 := by
  simp [minOperations_precond, List.range, List.map, List.all, List.elem]
use h_precond
simp only [minOperations_postcond]
-- Compute the result of minOperations
have h_result : minOperations [1, 2] 2 h_precond = 1 := by
  simp only [minOperations]
  simp only [ite_eq_right_iff]
  intro h_eq
  norm_num at h_eq
  rfl
rw [h_result]
-- Now we need to show the postcondition is false
simp only [List.reverse, List.take, List.range, List.map, List.all, List.elem]
-- The goal should be ¬(collected_all ∧ is_minimal)
-- where collected_all checks if [2] contains both 1 and 2
-- Since [2] doesn't contain 1, collected_all is false
simp only [ite_eq_left_iff, not_lt, le_refl, not_true, List.elem, or_false, eq_self_iff_true, true_and, not_and, not_not]
norm_num
