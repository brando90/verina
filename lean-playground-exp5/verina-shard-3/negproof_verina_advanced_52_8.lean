...
-- Compute what minOperations returns
have h_result : minOperations [1, 2] 2 h_precond = 1 := by
  simp [minOperations]
  rfl
rw [h_result]
-- Now show the postcondition is false
-- processed = [1, 2].reverse.take 1 = [2, 1].take 1 = [2]
-- target_nums = [1, 2]
-- collected_all requires both 1 and 2 to be in [2]
-- But [2] only contains 2, not 1, so collected_all is false
simp [List.reverse, List.take, List.range, List.map, List.all, List.elem]
-- The goal is now ¬(2 = 1 ∨ 2 = 2 ∧ (1 = 1 ∨ 1 = 2) ∧ ...)
-- This simplifies to ¬(False ∨ True ∧ (True ∨ False) ∧ ...)
-- = ¬(True ∧ True ∧ ...) which depends on the rest
-- But we know 1 is not in [2], so the first part of collected_all fails
simp
