...
use h_precond
simp [minOperations_postcond]
-- Compute the result of minOperations
have h_result : minOperations [1, 2] 2 h_precond = 1 := by
  simp [minOperations]
  simp [minOperations.count]
  rfl
rw [h_result]
-- Now check the postcondition with result = 1
simp [List.reverse, List.take, List.range, List.map, List.all, List.elem]
-- The goal is now: ¬(2 = 1 ∨ 2 = 2 ∧ (1 = 1 ∨ 1 = 2) ∧ (1 > 0 → ¬(2 = 1 ∧ 1 = 1)))
-- This simplifies to: ¬(True ∧ True ∧ (True → ¬False))
-- Which is: ¬(True ∧ True ∧ True) = ¬True = False
norm_num
