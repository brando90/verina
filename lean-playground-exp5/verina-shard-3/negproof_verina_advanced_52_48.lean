...
theorem minOperations_spec_violated : ∃ nums k,
  ...
  use [1, 2, 3], 2
have h_pre : minOperations_precond [1, 2, 3] 2 := by
...
have count_eq : minOperations.count [1, 2, 3] 0 = 1 := by
...
left
-- Show collected_all is false with result = 1
simp only [List.range, List.map]
have rev_eq : List.reverse [1, 2, 3] = [3, 2, 1] := by rfl
rw [rev_eq]
have take_eq : List.take 1 [3, 2, 1] = [3] := by rfl
rw [take_eq]
-- Need to show not all of [1, 2] are in [3]
