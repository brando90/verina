...
have count_eq : minOperations.count [2, 1] 0 = 1 := by
  ...
  simp only [minOperations.count]
rw [count_eq]
-- Now we need to show the postcondition is violated with result = 1
-- reverse [2, 1] = [1, 2], take 1 [1, 2] = [1]
-- But we need both 1 and 2, so collected_all is false
simp only [List.reverse, List.take, List.range, List.map, List.all, List.elem]
simp only [decide_True, decide_False, and_false]
