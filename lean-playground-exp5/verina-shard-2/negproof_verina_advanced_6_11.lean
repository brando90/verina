...
have impl_true : allVowels "aaa" trivial = true := by
  ...
  decide
-- The postcondition says: result ↔ normalized string contains all vowels
-- Since result = true, we need normalized "aaa" to contain all vowels
rw [impl_true] at h
simp only [normalize_str, String.toList] at h
-- Now h states that ['a', 'a', 'a'] contains all of ['a', 'e', 'i', 'o', 'u']
-- But ['a', 'a', 'a'] doesn't contain 'e'
have no_e : ¬(['a', 'a', 'a'].contains 'e') := by
  simp only [List.contains, List.elem]
  decide
-- From h, all vowels must be in ['a', 'a', 'a'], including 'e'
