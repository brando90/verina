...
theorem maxSubarraySum_spec_violated : ∃ numbers,
  ...
  use [-1], trivial
simp only [maxSubarraySum_postcond, maxSubarraySum]
simp only [maxSubarraySum.isAllNegative]
simp only [List.length, List.range, List.flatMap, List.map, List.drop, List.take,
           List.sum, List.contains, List.all, List.foldl, decide, List.getElem!,
           Nat.succ_eq_add_one, Nat.sub_self, Nat.sub_zero]
norm_num
