...
have h_zero_in : 0 ∈ (List.range ([-1].length + 1)).flatMap (fun start =>
  ...
  simp only [List.length_singleton, Nat.add_one, List.range_two, List.flatMap_cons,
             List.flatMap_singleton, List.flatMap_nil, List.append_nil]
  simp only [Nat.sub_zero, Nat.sub_self, List.range_one, List.range_two,
             List.map_cons, List.map_singleton, List.map_nil]
  simp only [List.drop_zero, List.take_zero, List.sum_nil, List.take_one,
             List.sum_singleton, List.drop_one, List.take_zero, List.sum_nil]
