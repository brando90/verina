...
have h_empty_sum : (List.nil : List Int).sum = 0 := by simp
-- Show that the empty subarray is in subArraySums
have h_zero_in : 0 ∈ (List.range ([-1].length + 1)).flatMap (fun start =>
  ...
  simp only [List.length, List.length_singleton]
  simp only [List.range, List.flatMap, List.map]
  simp only [List.mem_cons, List.mem_singleton]
  left
  -- When start = 0 and len = 0, we get empty list
