...
have all_in : List.all ['a', 'e', 'i', 'o', 'u'] (fun v => ['a', 'a', 'a'].contains v) :=
  h.mp trivial
-- But this is false because 'e' is not in ['a', 'a', 'a']
have not_all_in : ¬(List.all ['a', 'e', 'i', 'o', 'u'] (fun v => ['a', 'a', 'a'].contains v)) := by
  simp [List.all, List.foldr, List.contains, List.elem]
  decide
-- Contradiction
exact not_all_in all_in
