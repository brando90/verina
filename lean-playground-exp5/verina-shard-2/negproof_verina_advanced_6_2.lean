...
use trivial
intro h
simp [allVowels_postcond, normalize_str] at h
simp [allVowels]
have : allVowels "aaa" trivial = true := by
  simp [allVowels, String.toList, List.all]
  simp [Char.isAlpha]
  decide
rw [this] at h
simp at h
have : ¬List.all ['a', 'e', 'i', 'o', 'u'] (fun v => ("aaa").toList.contains v) := by
