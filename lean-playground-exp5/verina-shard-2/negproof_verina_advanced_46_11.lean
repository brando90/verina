...
-- The implementation returns -1 for the input [-1]
...
have h_zero_in_sums : 0 ∈ (List.range ([-1].length + 1)).flatMap (fun start =>
  ...
  simp only [List.length_singleton, List.range_succ_eq_map, List.flatMap_cons,
    List.flatMap_nil, List.append_nil, List.map_cons, List.map_nil,
    List.mem_cons, List.mem_singleton]
  left
  simp only [tsub_zero, List.range_succ_eq_map, List.map_cons, List.map_nil,
    List.drop_zero, List.take_zero, List.sum_nil]

-- From h_all, we get 0 ≤ -1
have h_contra := h_all 0 h_zero_in_sums
