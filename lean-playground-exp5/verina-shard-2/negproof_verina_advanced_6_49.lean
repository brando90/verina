...
theorem allVowels_spec_violated : ∃ s,
  ...
  use "a"
use trivial
unfold allVowels_postcond allVowels normalize_str
simp only [String.toList]
-- Show that the implementation returns true
...
rw [impl_true]
simp only [true_iff]
-- Show the postcondition requires false
simp [List.filter, List.map, Char.isAlpha, Char.toLower, List.all, List.contains, List.elem]
