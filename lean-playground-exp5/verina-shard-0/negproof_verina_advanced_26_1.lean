...
theorem letterCombinations_spec_violated : ∃ digits,
  ...
  use "1"
use trivial
simp [letterCombinations_postcond, letterCombinations]
have h1 : String.toList "1" = ['1'] := by rfl
simp [h1]
have h2 : List.any ['1'] (fun c => ¬(c ∈ ['2', '3', '4', '5', '6', '7', '8', '9'])) = true := by
  simp [List.any]
  decide
simp [h2]
-- Now we need to show that letterCombinations "1" trivial ≠ []
have h3 : letterCombinations "1" trivial = [""] := by
  simp [letterCombinations]
  rfl
simp [h3]
decide
