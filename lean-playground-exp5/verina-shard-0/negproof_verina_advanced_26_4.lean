...
simp only [String.isEmpty, String.toList]
-- "1" is not empty
have h_not_empty : ¬List.isEmpty ['1'] := by simp
simp [h_not_empty, ite_false]
-- "1" contains an invalid digit
have h_invalid : List.any ['1'] (fun c => ¬(c ∈ ['2', '3', '4', '5', '6', '7', '8', '9'])) = true := by
  simp [List.any]
  decide
simp [h_invalid, ite_true]
-- The postcondition requires the result to be []
-- But the implementation returns [""]
unfold letterCombinations
simp [String.toList]
-- The combine function is called with ['1'] and [""]
-- Since '1' is not in the mapping, combine returns acc unchanged
-- So the result is [""]
decide
