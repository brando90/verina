...
use trivial
intro h_post
-- The postcondition states that for invalid input "1", result should be []
have h_not_empty : ¬"1".isEmpty := by
  simp [String.isEmpty]
have h_invalid : "1".toList.any (fun c => ¬(c ∈ ['2', '3', '4', '5', '6', '7', '8', '9'])) := by
  simp [String.toList, List.any]
  -- '1' is not in the valid digits list
  simp [List.elem_iff, List.mem_iff_elem]
  decide
-- From the postcondition with these conditions
simp only [letterCombinations_postcond] at h_post
simp [h_not_empty, h_invalid, if_neg, if_pos] at h_post
-- h_post now states: letterCombinations "1" trivial = []
-- Let's compute what the implementation actually returns
