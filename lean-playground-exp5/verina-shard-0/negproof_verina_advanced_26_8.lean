...
use trivial
intro h_post
-- The postcondition says that for invalid digits, result should be []
have h_not_empty : ¬"1".isEmpty := by
  simp [String.isEmpty]
have h_invalid : "1".toList.any (fun c => ¬(c ∈ ['2', '3', '4', '5', '6', '7', '8', '9'])) := by
  simp [String.toList, List.any]
  decide
-- So the postcondition says result = []
simp only [letterCombinations_postcond, h_not_empty, h_invalid, if_neg, if_pos] at h_post
...
have h_impl : letterCombinations "1" trivial = [""] := by
  ...
  -- The combine function processes '1' which is not in mapping
  -- So it just returns the accumulator [""] unchanged
  unfold letterCombinations.combine
  simp only [List.find?]
  rfl
-- This contradicts h_post which says result = []
rw [h_impl] at h_post
-- [""] ≠ []
cases h_post
