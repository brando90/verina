...
use trivial
intro h_post
-- The specification says for invalid digit "1", result should be []
...
have h_invalid : "1".toList.any (fun c => ¬(c ∈ ['2', '3', '4', '5', '6', '7', '8', '9'])) := by
  ...
  decide
-- Apply postcondition
simp [letterCombinations_postcond, h_not_empty, h_invalid] at h_post
-- h_post now states letterCombinations "1" trivial = []
-- But let's verify what the implementation actually returns
have h_impl : letterCombinations "1" trivial = [""] := by
  rfl
-- Contradiction
rw [h_impl] at h_post
-- h_post is [""] = [] which is false
