...
use trivial
intro h_post
-- The postcondition for invalid input "1" should give us []
...
have h_invalid : "1".toList.any (fun c => ¬(c ∈ ['2', '3', '4', '5', '6', '7', '8', '9'])) := by
  ...
  decide
-- By the postcondition, result should be []
have h_spec : letterCombinations "1" trivial = [] := by
  simp [letterCombinations_postcond] at h_post
  simp [h_not_empty, h_invalid] at h_post
  exact h_post
-- But the implementation returns [""]
have h_impl : letterCombinations "1" trivial = [""] := by
  simp [letterCombinations, String.toList]
  rfl
-- This is a contradiction
rw [h_impl] at h_spec
exact List.noConfusion h_spec
