...
have h_prec : 1 ≤ [1].length ∧ [1].length ≤ 100 ∧ [1].all (fun x => 1 ≤ x ∧ x ≤ 100) := by
  simp [List.all]
specialize h h_prec
obtain ⟨h_ge, _, h_eq⟩ := h
simp [solution] at h_eq
-- Implementation computes: 1*(1+1)/2 - 1 = 1 - 1 = 0
-- But the spec expects the sum of squares of distinct counts
-- For [1], we have one subarray [1] with 1 distinct element, so expected = 1² = 1
-- This gives us 0 = 1, which is false
norm_num at h_eq
