...
use trivial
intro h_post
-- The specification says if digits contains any invalid character, result should be []
-- '1' is not in ['2','3','4','5','6','7','8','9'], so result should be []
have h_invalid : "1".toList.any (fun c => ¬(c ∈ ['2', '3', '4', '5', '6', '7', '8', '9'])) := by
  simp [String.toList, List.any, List.mem_cons, List.mem_singleton]
  decide
-- The string "1" is not empty
have h_not_empty : ¬"1".isEmpty := by
  decide
-- From the postcondition with these facts
...
have h_result : letterCombinations "1" trivial = [""] := by
  rfl
rw [h_result] at h_post
-- Now h_post says [""] = [], which is clearly false
cases h_post
