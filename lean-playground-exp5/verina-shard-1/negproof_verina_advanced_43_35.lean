...
use 2
constructor
· -- Show that 2 is in the products list
  simp only [List.mem_map, List.mem_filter, List.mem_sublists]
  use [-1, -2]
  constructor
  · constructor
    · -- [-1, -2] is a sublist of [-1, -2]
  ...
  · -- Product of [-1, -2] is 2
    simp [List.foldl]
    norm_num
· -- Show that maxStrength returns -1 < 2
  simp only [not_le]
  -- Evaluate maxStrength [-1, -2]
  have h_calc : maxStrength [-1, -2] h_precond = -1 := by
    unfold maxStrength
    simp [List.filter, List.isEmpty, List.length, List.foldl, max, List.getElem!]
    norm_num
  rw [h_calc]
  norm_num
