...
theorem maxStrength_spec_violated : ∃ nums,
  ...
  use [-2, -1]
use (by simp [maxStrength_precond] : maxStrength_precond [-2, -1])
simp [maxStrength_postcond]
push_neg
-- Compute what maxStrength actually returns
have h_result : maxStrength [-2, -1] _ = -1 := by
  simp [maxStrength, List.filter, List.foldl, List.isEmpty, List.length]
  norm_num
  rfl
rw [h_result]
-- Now show that either -1 is not in the products or there's a product > -1
right
-- The sublists are [[-2], [-1], [-2, -1]] and products are [-2, -1, 2]
use 2
constructor
