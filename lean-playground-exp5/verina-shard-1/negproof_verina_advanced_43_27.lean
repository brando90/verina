...
use 12
constructor
· -- Show that 12 is in the products list (from sublist [-3, -4])
  simp only [List.mem_map, List.mem_filter, List.mem_sublists]
  use [-3, -4]
  constructor
  · constructor
    · decide  -- [-3, -4] is a sublist of [-2, -3, -4]
    · decide  -- [-3, -4] ≠ []
  · simp [List.foldl]
    norm_num  -- (-3) * (-4) = 12
· -- Show that maxStrength returns -2 but 12 > -2
  constructor
  · -- Compute what maxStrength actually returns
    rw [maxStrength]
    simp only [List.filter, List.length, List.foldl, List.isEmpty, List.getElem!, List.get!]
    decide
  · -- 12 > -2
    norm_num
