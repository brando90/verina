...
· -- Show that 12 is in the products list
  ...
  · constructor
    · -- [-3, -4] is a sublist of [-2, -3, -4]
      simp only [List.Sublist]
      apply List.Sublist.cons
      apply List.sublist_cons_of_sublist
      apply List.sublist_refl
    · -- [-3, -4] ≠ []
      decide
...
· -- Show that maxStrength returns -2 < 12
  simp only [not_le]
  -- Evaluate maxStrength [-2, -3, -4]
  have h_calc : maxStrength [-2, -3, -4] h_precond = -2 := by
    simp only [maxStrength]
    simp [List.filter, List.isEmpty, List.length, List.foldl]
    norm_num
    -- The max branch evaluates to -2
    rfl
  rw [h_calc]
  norm_num
